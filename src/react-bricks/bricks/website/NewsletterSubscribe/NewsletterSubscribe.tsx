import * as React from 'react'
import jsonp from 'jsonp'
import { validate } from 'email-validator'
import classNames from 'classnames'

import { Text, types } from 'react-bricks/frontend'
import blockNames from '../blockNames'

enum NewsletterProvider {
  MailChimp = 'MAILCHIMP',
  ConvertKit = 'CONVERTKIT',
}

export interface NewsletterSubscribeProps {
  centered?: boolean
  provider: NewsletterProvider
  mailchimpUrl: string
  buttonText: string
  resultOkText: string
}

interface IStatus {
  status: string
  message: string
}

const NewsletterSubscribe: types.Brick<NewsletterSubscribeProps> = ({
  centered = false,
  provider,
  mailchimpUrl,
  buttonText,
  resultOkText = `Dziękujemy, zostaniemy w kontakcie!`,
}) => {
  const [email, setEmail] = React.useState('')
  const [status, setStatus] = React.useState<IStatus>({
    status: 'IDLE',
    message: '',
  })

  const sendData = (url: string) => {
    setStatus({ status: 'SENDING', message: '' })
    jsonp(url, { param: 'c', timeout: 3500 }, (err: any, data: any) => {
      if (err) {
        setStatus({
          status: 'ERROR',
          message: 'Wystąpił błąd. Proszę spróbuj ponownie.',
        })
      } else if (data.msg.includes('already subscribed')) {
        setStatus({
          status: 'ERROR',
          message: 'Już wcześniej zasubskrybowałeś!',
        })
      } else if (data.result !== 'success') {
        setStatus({
          status: 'ERROR',
          message: 'Wystąpił błąd. Proszę spróbuj ponownie.',
        })
      } else {
        setStatus({ status: 'SUCCESS', message: '' })
      }
    })
  }

  const handleSubmit = (event: React.FormEvent) => {
    event.preventDefault()
    if (provider !== NewsletterProvider.MailChimp) {
      setStatus({
        status: 'ERROR',
        message: 'Dostawca nie został zaimplementowany',
      })
      return
    }
    const isEmailValid = validate(email)

    if (!isEmailValid) {
      setStatus({
        status: 'ERROR',
        message: 'Proszę wprowadzić prawidłowy adres e-mail!',
      })
      return
    }

    if (
      !mailchimpUrl ||
      mailchimpUrl.length < 10 ||
      mailchimpUrl.indexOf('post') === -1
    ) {
      setStatus({
        status: 'ERROR',
        message: 'Nieprawidłowy adres Mailchimp',
      })
      return
    }

    const emailEncoded = encodeURIComponent(email)
    const endpoint = mailchimpUrl.replace(/\/post/g, '/post-json')
    const url = `${endpoint}?EMAIL=${emailEncoded}`
    sendData(url)
  }

  return (
    <section
      className="py-12"
      style={{ backgroundColor: '#deeffc', color: '#113d5f' }}
    >
      <div
        className={classNames('max-w-xl mx-auto flex flex-col', {
          'items-center': centered,
        })}
      >
        <Text
          renderBlock={(props) => (
            <h1 className="text-2xl mb-4 font-extrabold" {...props.attributes}>
              {props.children}
            </h1>
          )}
          placeholder="Wprowadź tytuł..."
          propName="title"
        />
        <Text
          renderBlock={(props) => (
            <p className="mb-2" {...props.attributes}>
              {props.children}
            </p>
          )}
          placeholder="Opis..."
          propName="description"
        />
        <form className="flex" onSubmit={handleSubmit}>
          <input
            className="bg-white focus:outline-none border-t-2 border-l-2 border-b-2 border-transparent focus:border-sky-500 rounded-l-lg py-2 px-4 appearance-none leading-normal"
            type="text"
            placeholder="jane@example.com"
            value={email}
            onChange={(e) => setEmail(e.target.value)}
          />
          <button
            type="submit"
            disabled={status.status === 'SENDING'}
            className="bg-sky-500 px-8 rounded-r-lg text-white font-bold py-2"
          >
            {buttonText}
          </button>
        </form>
        {status.status === 'SUCCESS' && (
          <div className="text-xl mt-4">{resultOkText}</div>
        )}
        {status.status === 'ERROR' && (
          <div className="mt-4" style={{ color: '#c00' }}>
            {status.message}
          </div>
        )}
      </div>
    </section>
  )
}

NewsletterSubscribe.schema = {
  name: blockNames.NewsletterSubscribe,
  label: 'Subskrybcja Newslettera',
  category: 'rb-ui website',

  getDefaultProps: () => ({
    title: 'Chcesz otrzymywać wiadomości od nas?',
    description: 'Zostaw swój adres e-mail!',
    provider: NewsletterProvider.MailChimp,
    buttonText: 'Subskrybuj',
    resultOkText: `Dziękujemy, będziemy z Tobą w kontakcie!`,
  }),
  sideEditProps: [
    {
      groupName: 'Newsletter',
      defaultOpen: true,
      props: [
        {
          name: 'centered',
          label: 'Wyśrodkowany',
          type: types.SideEditPropType.Boolean,
        },
        {
          name: 'mailchimpUrl',
          label: 'Adres formularza Mailchimp',
          type: types.SideEditPropType.Text,
          validate: (value) =>
            value && value.length > 10 && value.indexOf('https://') !== -1,
          //&& value.indexOf('list-manage.com/subscribe/post?') !== -1,
        },
        {
          name: 'buttonText',
          label: 'Tekst przycisku',
          type: types.SideEditPropType.Text,
        },
        {
          name: 'resultOkText',
          label: 'Treść pozytywnej wiadomości',
          type: types.SideEditPropType.Text,
        },
      ],
    },
  ],
}

export default NewsletterSubscribe
